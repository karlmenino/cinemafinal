...
import io.menino.demo.dao.FilmTmbdDao;
...
public class TmdbManager {
    ...
    public Personne importPersonne(JSONObject role,Film filma){
        ...
        if (!personManager.existsByIdtmbd(BigIntegerId)){
            ...
            if (ann2.length()!=0){
                personne.setNaissance (LocalDate.parse(ann2));
            }
            ...
            personne.setNom (person.optString("name"));
    ...
    public Film peuplerFilm(BigInteger id) throws InterruptedException {
        ...
        if (!filmManager.existsByIdtmbd(film.getBigInteger("id"))){
            ...
            if (ann.length()!=0){
                filma.setRelease (LocalDate.parse(ann));
            }
            ...
            for(int i = 0; i < genres.length(); i++){
                ...
                Set<Genre> lesgenres = filma.getGenreFilm ();
                if (lesgenres!=null){
                    lesgenres.add(gen);
                    filma.setGenreFilm (lesgenres);
                }
                else {
        ...
        for (int i = 0; i < cast.length(); i++ ) {
            ...
            Personne personne = importPersonne(role,filma);
            if (!roleManager.existsByNameAndFilmAndPerson (role.optString("character"),personne,filma)){
                Role play = new Role ();
                play.setName(role.optString("character"));
                play.setRank(role.optInt("order"));
                play.setPerson(personne);
                play.setFilm(filma);
        ...
        for (int i = 0; i < crew.length(); i++ ) {
            ...
            if (role.getString("job").equals("Director")) {
                Personne personne = importPersonne(role,filma);
                filma.setFilm_director(personne);
